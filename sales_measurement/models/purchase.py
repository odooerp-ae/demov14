...
class Purchase(models.Model):
    ...
    sale_order_ids = fields.Many2many('sale.order', string="Sales", compute='_compute_sale_orders')

    @api.depends('order_line.sale_order_id','order_line.move_dest_ids.group_id.mrp_production_ids',
                 'order_line.move_ids.move_dest_ids.group_id.mrp_production_ids')
    def _compute_sale_orders(self):
        for purchase in self:
            mrp_production_ids = (
                purchase.order_line.move_dest_ids.group_id.mrp_production_ids | purchase.order_line.move_ids.move_dest_ids.group_id.mrp_production_ids)
            if mrp_production_ids:
                sale_order_ids = mrp_production_ids.procurement_group_id.mrp_production_ids.move_dest_ids.group_id.sale_id.ids
                purchase.sale_order_ids = [(6, 0, sale_order_ids)]
            else:

                purchase.sale_order_ids = [(6, 0, purchase._get_sale_orders().ids)]
